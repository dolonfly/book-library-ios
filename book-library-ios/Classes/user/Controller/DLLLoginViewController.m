...
@interface DLLLoginViewController ()<UITextFieldDelegate>
{
    UITextField* txtUser;
    UITextField* txtPwd;
...
@implementation DLLLoginViewController
...
- (void)viewDidLoad {
    ...
    txtPwd.tag = 1;
    [txtPwd addTarget:self action:@selector(nextOnKeyboard:) forControlEvents:UIControlEventEditingDidEndOnExit];
...
- (void)nextOnKeyboard:(id) sender
{
    ...
    if (![username isEqual:@"admin"]&&![password isEqual:@"admin"]) {
        ...
        alert.title = @"账号或密码不正确";
        [alert show];
    }else{
        DLLUser *u = [DLLUser new];
        [u login];
